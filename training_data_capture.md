...
*   Fly in a perimeter around the fall zone and take 50-100 images.
...
_Can I skip false training data collection at one site?_ **NO**, the prediction model will perform much better if trained on the local background (vegetation, rock types, animals...). So far we have not seen two fall sites that are similar enough to a point where they where can use the same background training data.
